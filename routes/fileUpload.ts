...
function handleZipFileUpload(
  ...
  if (utils.endsWith(file?.originalname.toLowerCase(), ".zip")) {
    if (
      file?.buffer != null &&
      utils.isChallengeEnabled(challenges.fileWriteChallenge)
    ) {
...
function handleXmlUpload({ file }: Request, res: Response, next: NextFunction) {
  if (utils.endsWith(file?.originalname.toLowerCase(), ".xml")) {
    ...
    if (
      file?.buffer != null &&
      utils.isChallengeEnabled(challenges.deprecatedInterfaceChallenge)
    ) {
      ...
      try {
        ...
        const xmlDoc = vm.runInContext(
          "libxml.parseXml(data, { noblanks: true, noent: false, dtdload: false, nocdata: true })",
          sandbox,
          { timeout: 2000 }
        );
  ...
  if (
    ...
    if (
      ...
      try {
        ...
        // TODO: Remove any
        if (
          utils.contains(err.message, "Invalid string length") ||
